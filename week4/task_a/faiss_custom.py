...
class FaissRetrieval(object):
    ...
    def get_embeddings(self, split):
        ...
        embedding_features = pkl.load(open(path, "rb"))
        embedding_features = np.array([item for sublist in embedding_features for item in sublist]).reshape(dim)
    ...
    def train(self):
        ...
        if ids.any():
            if not self.index.is_trained and self.INDEX_KEY != "IDMap,Flat":
                self.index.train(np.ascontiguousarray(features))
                print('Training: DONE')
                self.index.add_with_ids(np.ascontiguousarray(features),np.ascontiguousarray(ids))
    ...
    def __search__(self, ids, vectors, k):
        ...
        for id_, feature in zip(ids, vectors):
            scores, neighbors = self.index.search(feature, k = k) if feature.size > 0 else ([], [])
